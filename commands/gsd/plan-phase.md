...
PHASE_GOAL=$(node /Users/gshah/.claude/get-shit-done/bin/gsd-tools.js roadmap get-phase "${PHASE}" 2>/dev/null | jq -r '.goal // empty')
...
if [ -n "$PAST_DECISIONS" ] || [ -n "$PAST_PATTERNS" ]; then
  # Check if results are not just "No past learnings found" messages

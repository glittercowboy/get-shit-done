...
must_haves:
  ...
  artifacts:
    ...
    - path: "mcp-servers/telegram-mcp/src/index.ts"
      provides: "Session-end analysis hook in SIGINT/SIGTERM handlers"
      contains: "analyzeBeforeClose"
...
3. **Deduplicate before insertion** using Phase 4 three-stage dedup (knowledge-dedup.js):
   ...
   - If near-duplicate (0.65-0.88): use knowledge-evolution.js `evolveMemory()` to update existing entry
...
<task type="auto">
  <name>Task 2: Wire session-end analysis hook into MCP server lifecycle</name>
  <files>
    mcp-servers/telegram-mcp/src/storage/session-manager.ts
    mcp-servers/telegram-mcp/src/index.ts
  </files>
...
After TypeScript changes, rebuild: `cd mcp-servers/telegram-mcp && npm run build`
  ...
  <done>
    MCP server runs session analysis BEFORE closing session on SIGINT/SIGTERM. Analysis is gated by quality checks and re-analysis prevention. Analysis failures do not prevent session close. 10-second timeout prevents hanging. TypeScript compiles without errors.
  </done>
</task>

...
<task type="auto">
  ...
  <action>
NOTE: The VERIFICATION.md previously marked gsd-task-router.md as VERIFIED, but this is stale — the file does not exist on disk. Create it fresh per the content below.
...
The coordinator is a markdown prose file, not a template engine. The replacement above changes a literal string in the coordinator's prose instructions. Concretely: the coordinator text currently reads (somewhere in the execute step): `Task(subagent_type="gsd-executor", model="sonnet", ...)`. After this change it should read: `Task(subagent_type="gsd-executor", model="{EXECUTOR_MODEL}", ...)` — where `{EXECUTOR_MODEL}` is a variable name in the coordinator's prose instructions that was set earlier in the step: either to the router's returned model tier (when auto profile is active) or to `"sonnet"` (when auto profile is not active).

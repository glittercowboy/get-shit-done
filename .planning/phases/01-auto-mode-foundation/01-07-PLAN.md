...
<task type="auto">
  ...
  <action>
In both gsd-tools.js files, locate the routing section (comment: "// Routing: model selection + context injection (Phase 01)").

After the `mergeRoutingRules()` function and BEFORE `selectModelFromRules()`, insert the `computeComplexityScore()` function.

Note: The two files may differ in line count — do NOT rely on line numbers for the insertion point. Find the location by searching for surrounding function context: `mergeRoutingRules()` ends immediately before the insertion point, and `selectModelFromRules()` begins immediately after.

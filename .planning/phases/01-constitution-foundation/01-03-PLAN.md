...
2. Create global CONSTITUTION.md at ~/.claude/get-shit-done/CONSTITUTION.md (ROOT LEVEL - not in templates/):
```javascript
// Create global constitution at root of GSD install directory
...
// Initialize global constitution at ROOT level (matches loader path expectation)
// Loader expects: ~/.claude/get-shit-done/CONSTITUTION.md
// NOT: ~/.claude/get-shit-done/templates/constitution/global-constitution.md
const constitutionPath = initGlobalConstitution(gsdDir);
console.log(`  ${green}✓${reset} Global constitution: ${constitutionPath}`);
```

**CRITICAL PATH REQUIREMENT:**
- Target path MUST be `path.join(homeDir, '.claude', 'get-shit-done')` (root level)
- Result file MUST be at `~/.claude/get-shit-done/CONSTITUTION.md`
- This matches loader.js path: `path.join(os.homedir(), '.claude', 'get-shit-done', 'CONSTITUTION.md')`
...
Constitution initialization called with `path.join(homeDir, '.claude', 'get-shit-done')` as target.
Success message printed with checkmark.
Code follows existing install.js style (ANSI colors, error handling).
Test: Run modified install.js logic (or dry-run) to verify constitution created at `~/.claude/get-shit-done/CONSTITUTION.md` (ROOT LEVEL).
  </verify>
  <done>
Install script updated in GSD install location.
Global constitution automatically installed to ~/.claude/get-shit-done/CONSTITUTION.md during GSD setup.
...
# Verify global constitution created at ROOT LEVEL
...
- Global CONSTITUTION.md at correct path: ~/.claude/get-shit-done/CONSTITUTION.md (ROOT LEVEL)
</verification>

<success_criteria>
- [ ] Constitution init helper created in GSD install location
- [ ] Install script updated in GSD install to create global CONSTITUTION.md at ROOT LEVEL
- [ ] Global CONSTITUTION.md created at ~/.claude/get-shit-done/CONSTITUTION.md (not in templates/)
...
- [ ] End-to-end test confirms loader can read created constitutions from correct paths
- [ ] Files copied from GSD install to repo source

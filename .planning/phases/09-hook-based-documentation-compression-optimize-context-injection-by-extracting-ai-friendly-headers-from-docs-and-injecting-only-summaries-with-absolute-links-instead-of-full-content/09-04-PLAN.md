...
<task type="auto">
  ...
  6. **compress clear-cache** - Clear compression cache:
  ```javascript
  function cmdCompressClearCache(raw) {
    const cachePath = path.join(HOME, '.claude', 'get-shit-done', 'compression-cache');
    if (fs.existsSync(cachePath)) {
      fs.rmSync(cachePath, { recursive: true });
    }
    output({ message: 'Compression cache cleared' }, raw);
  }
  ```

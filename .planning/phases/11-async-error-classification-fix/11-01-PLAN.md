...
must_haves:
  truths:
    ...
    - "classifyError returns TIMEOUT for SIGTERM signal (both paths)"
...
<task type="auto">
  <name>Task 1: RED — Add failing classifyError unit tests for all 3 adapters</name>
...
Create a shared test suite pattern — for EACH adapter, test these 7 cases:
...
5. **SIGTERM** — `{ signal: 'SIGTERM', code: null }` → `'TIMEOUT'`
6. **ENOENT string** — `{ code: 'ENOENT', signal: null }` → `'NOT_FOUND'`
7. **other exit codes** — `{ status: 1, signal: null }` → `'EXIT_ERROR'` AND `{ code: 42, signal: null }` → `'EXIT_ERROR'`

Use `describe('classifyError', () => { ... })` as outer block, with nested `describe` per adapter: `describe('codex adapter', () => { ... })`.

Use `assert.strictEqual` for all assertions (matching existing test patterns).

NOTE: Tests for cases 2 and 4 MUST fail before the fix — this confirms the bug exists. Case 7 async shape (`{ code: 42 }`) should also return EXIT_ERROR (not misclassified).
  </action>
  <verify>
Run `node --test get-shit-done/bin/gsd-tools.test.cjs 2>&1 | grep -E '(pass|fail|classifyError)'` — expect test cases 2 and 4 (async shape 127 and 126) to FAIL for all 3 adapters. All other existing tests must still pass.
  </verify>
  <done>
classifyError tests exist for all 3 adapters with 7 test cases each (21 total). Async shape tests (cases 2, 4) fail, confirming the bug. Sync shape tests and all pre-existing tests pass.
  </done>
</task>

<task type="auto">
  <name>Task 2: GREEN — Fix classifyError in all 3 adapters and export it</name>
  <files>
    get-shit-done/bin/adapters/codex.cjs
    get-shit-done/bin/adapters/gemini.cjs
    get-shit-done/bin/adapters/opencode.cjs
  ...
  <action>
In each of the 3 adapter files, apply the identical fix to `classifyError`:
...
Then update `module.exports` in each file to export `classifyError`:
...
Run `node --test get-shit-done/bin/gsd-tools.test.cjs` — ALL tests must pass, including the previously-failing async shape tests. Zero failures.
  ...
  <done>
All 3 adapters have the fixed classifyError (handles both err.status and numeric err.code). classifyError is exported from module.exports in all 3 files. All tests pass — async error shapes correctly classified as NOT_FOUND (127) and PERMISSION (126).
  </done>
</task>
...
<success_criteria>
- All classifyError unit tests pass for all 3 adapters (21 test cases)

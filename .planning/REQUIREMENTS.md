...
- [ ] **EXEC-09**: Checkpoint state after each significant action using structured format (task, plan, progress, decisions, next_steps)
- [ ] **EXEC-10**: Resume from last checkpoint on failure via semantic search of checkpoint memories
...
- [ ] **KNOW-01**: Local database using SQLite + sqlite-vec (better-sqlite3 binding) for combined relational and vector storage
...
- [ ] **KNOW-06**: Multi-phase search pipeline: vector similarity → FTS5 text → type weights → context boost
- [ ] **KNOW-07**: Type-weighted scoring (decisions/lessons rank 2.0x, summaries rank 0.5x)
- [ ] **KNOW-08**: TTL categories for memory lifecycle (permanent: lessons/patterns, long-term: decisions, short-term: summaries, ephemeral: temp notes)
- [ ] **KNOW-09**: Automatic cleanup of expired memories based on TTL category
- [ ] **KNOW-10**: Knowledge has staleness tracking (timestamps, volatility scores)
- [ ] **KNOW-11**: Access tracking on every retrieval (access_count, last_accessed) for relevance ranking
- [ ] **KNOW-12**: On-the-fly extraction during GSD flows (Haiku agent)
- [ ] **KNOW-13**: Q&A sessions: Claude generates questions, user answers, system learns
- [ ] **KNOW-14**: Session scanning: batch review of past conversations
- [ ] **KNOW-15**: Synthesis passes: consolidate knowledge into principles
- [ ] **KNOW-16**: Three-stage deduplication: content hash → canonical hash → embedding similarity (0.88 threshold)
- [ ] **KNOW-17**: Memory evolution: when similarity 0.65-0.88, update existing memory instead of creating duplicate
- [ ] **KNOW-18**: Principles extracted from knowledge (higher-level reasoning patterns)
- [ ] **KNOW-19**: Claude makes autonomous decisions based on learned principles
- [ ] **KNOW-20**: Stop-and-ask for irreversible actions (data/code deletion)
- [ ] **KNOW-21**: Stop-and-ask for external communications (not to user)
- [ ] **KNOW-22**: Stop-and-ask for actions that cost money
- [ ] **KNOW-23**: Allow autonomous execution when explicitly permitted with boundaries
- [ ] **KNOW-24**: Track explicit permissions with stated limits (e.g., "max $20")
- [ ] **KNOW-25**: Conflict resolution: when principles conflict, user-defined priority rules
- [ ] **KNOW-26**: Feedback loop: user can mark "this principle was wrong/outdated"
- [ ] **KNOW-27**: Learn from feedback to update/invalidate principles

### Hooks Integration (moved to Phase 4 - foundational for extraction)

- [ ] **HOOK-01**: Claude hooks capture conversation context via regex patterns for decisions/lessons
- [ ] **HOOK-02**: Auto-capture patterns detect decisions (e.g., "let's use X", "decided to Y", "going with Z")
- [ ] **HOOK-03**: Auto-capture patterns detect lessons (e.g., "I learned", "turns out", "the trick is")
- [ ] **HOOK-04**: Quality gates prevent noise (min length 20 chars, tech signal detection)
- [ ] **HOOK-05**: Configurable timing: per-turn mode (analyze after each response)
- [ ] **HOOK-06**: Configurable timing: session-end mode (batch process at end)
- [ ] **HOOK-07**: Enable/disable configuration for hooks capture
- [ ] **HOOK-08**: Switch between per-turn and session-end when enabled
...
- **KNOW-A1**: Graph edges for memory relationships (related, supersedes, contradicts)
...
- **KNOW-A4**: Encryption at rest for sensitive knowledge
...
| Custom embedding model training | Use pre-trained models (bge-small-en-v1.5) |
| Separate MCP server for knowledge | GSD uses skills, not MCP tools |
...
| KNOW-13 | Phase 4 | Pending |
| KNOW-14 | Phase 4 | Pending |
| KNOW-15 | Phase 4 | Pending |
| KNOW-16 | Phase 4 | Pending |
| KNOW-17 | Phase 4 | Pending |
| KNOW-18 | Phase 4 | Pending |
| KNOW-19 | Phase 4 | Pending |
| KNOW-20 | Phase 5 | Pending |
| KNOW-21 | Phase 5 | Pending |
| KNOW-22 | Phase 5 | Pending |
| KNOW-23 | Phase 5 | Pending |
| KNOW-24 | Phase 5 | Pending |
| KNOW-25 | Phase 5 | Pending |
| KNOW-26 | Phase 5 | Pending |
| KNOW-27 | Phase 5 | Pending |
...
*Last updated: 2026-02-15 after omega-memory research synthesis*
